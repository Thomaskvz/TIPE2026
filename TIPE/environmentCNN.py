...
class Environment:
    ...
    def process_image(self):
        try:
            # --- AJOUT : On vide le buffer pour récupérer l'image EN TEMPS RÉEL ---
            self.udp_socket.setblocking(False)
            last_data = None
            while True:
                try:
            ...
            # Si on a récupéré des paquets, on prend le plus récent, sinon on attend le suivant
            ...
            if self.last_frame is not None:
                ...
                img = self.last_frame[self.debutimg:, :].flatten() / 255
